...
const drawerContentVariants = cva(
  'bg-background-100 fixed z-50 flex flex-col shadow-lg will-change-transform outline-none',
  {
    variants: {
      side: {
        bottom: 'border-base-100 inset-x-0 bottom-0 rounded-t-xl border-t',
        top: 'border-base-100 inset-x-0 top-0 rounded-b-xl border-b',
        right:
          'border-base-100 inset-y-0 right-0 h-full w-full max-w-sm rounded-l-xl border-l',
        left: 'border-base-100 inset-y-0 left-0 h-full w-full max-w-sm rounded-r-xl border-r',
      },
    },
    defaultVariants: { side: 'bottom' },
  },
      ...
      {isOpen && (
        ...
        >
          ...
          >
            ...
            <AriaDialog
              className="flex flex-1 flex-col gap-4 overflow-y-auto outline-none"
              {...props}
            >
              {({ close }) => (
                <>
                  ...
                  {showCloseButton && (
                    <AriaButton
                      onPress={close}
                      aria-label="Close"
                      className="text-text-100 focus-visible:ring-primary-500 absolute top-4 right-4 flex size-7 items-center justify-center rounded-md opacity-70 transition-opacity outline-none hover:opacity-100 focus-visible:ring-2 disabled:pointer-events-none [&>svg]:size-5"
                    >
                      <Close />
...
const handleVariants = cva(
  'bg-foreground-soft-50 hover:bg-base-300 shrink-0 rounded-full transition-colors',
  {
    variants: {
      side: {
        bottom: 'mx-auto mt-3 mb-1 h-1.5 w-12',
        top: 'order-last mx-auto mt-1 mb-3 h-1.5 w-12',
        right:
          'absolute top-1/2 left-1.5 order-last h-12 w-1.5 -translate-y-1/2',
        left: 'absolute top-1/2 right-1.5 order-last h-12 w-1.5 -translate-y-1/2',
      },
    },
    defaultVariants: { side: 'bottom' },
  },
...
export function DrawerTitle({ className, ...props }: DrawerTitleProps) {
  return (
    <Heading
      slot="title"
      className={cn(
        'text-title-50 text-lg leading-none font-semibold',
        className,
      )}
      {...props}
    />
...
export function DrawerDescription({
  ...
  return (
    <p
      ...
      className={cn('text-text-100 text-sm', className)}
      {...props}
    />
  );
}
...
export function DrawerBody({ className, ...props }: DrawerBodyProps) {
  return (
    <div
      ...
      className={cn('text-text-100 flex-1 overflow-y-auto text-sm', className)}
      {...props}
    />
  );
}
...
export function DrawerFooter({
  ...
  return (
    ...
    >
      {showCloseButton && (
        <DrawerClose className="border-base-100 bg-background-100 text-title-50 hover:bg-background-soft-50 focus-visible:ring-primary-500 inline-flex h-10 items-center justify-center rounded-lg border px-4 text-sm font-medium transition outline-none focus-visible:ring-2">
          Close
        </DrawerClose>
      )}
      {children}
