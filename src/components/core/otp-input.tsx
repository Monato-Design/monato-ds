...
export default function OtpInput({
  ...
  return (
    <div>
      ...
      <div className="not-focus-within:text-input-placeholder-text focus-within:text-text-50 flex items-center gap-2">
        {otp.map((digit, index) => (
          <input
            key={index}
            type="text"
            ...
            className={cn(
              'bg-input-background focus:border-input-primary-focus-border focus:ring-input-primary-focus-border/20 border-base-300 disabled:border-base-100 disabled:bg-background-soft-50 disabled:text-input-disabled-text flex size-11 items-center justify-center rounded-lg border p-2 text-center text-base font-normal shadow-xs focus:ring-3 focus:outline-none',
              digit && 'text-text-50',
              className,
            )}
            style={{
        ...
        {/* Divider */}
        {digitLength === 6 && (
          <div className="bg-background-soft-500 order-3 mx-2 h-0.5 w-3 shrink-0 rounded-full" />
        )}
      </div>

      {hint && <p className="text-text-50 mt-2 text-sm">{hint}</p>}
    </div>
  );
}
