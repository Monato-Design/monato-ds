...
import {
  Home2StrokeRounded, DollarCircleStrokeRounded, UserMultiple4StrokeRounded,
...
function CurrencySelector({
  ...
  return (
    <div className="relative">
      ...
      >
        {selected
          ? <><CurrencyToken currency={selected} /><span className="text-title-50 text-sm font-medium">{selected.code}</span></>
        ...
        {open && (
          <>
            ...
            <motion.div
              ...
              className="absolute top-full left-0 mt-2 w-56 rounded-xl border border-base-100 bg-background-50 shadow-xl z-20 p-1.5"
            >
              {options.map(c => (
                <button
                  key={c.code}
          ...
          <div key={step.id} className="flex items-center gap-2">
            <motion.div className="flex items-center gap-2">
              <motion.div
                initial={false}
                animate={{
                  backgroundColor: isPending ? '#f4f4f5' : '#0894c8',
                  scale: isActive ? 1.05 : 1,
                }}
                transition={{ type: 'spring', stiffness: 400, damping: 25 }}
                className={`size-6 rounded-full flex items-center justify-center text-[11px] font-semibold ${
                  isPending ? 'text-text-200' : 'text-white'
                }`}
              >
                {isCompleted
                  ? <span className="[&_svg]:fill-none [&_path]:fill-none"><CheckStrokeRounded size={11} strokeWidth={2.2} /></span>
                  : step.id
                }
              </motion.div>
            ...
            {i < STEPS.length - 1 && (
              <div className={`w-12 h-px transition-colors ${
                step.id < current ? 'bg-primary-500' : 'bg-base-200'
              }`} />
            )}
          </div>
        );
...
function QuoteScreen({ state, dispatch }: { state: FlowState; dispatch: React.Dispatch<FlowAction> }) {
  ...
  return (
    <div className="max-w-md w-full mx-auto py-10">
      <h2 className="text-title-50 text-xl font-semibold">Currency Exchange</h2>
      <p className="text-text-100 text-sm mt-1">Type in either field to get a quote.</p>

      <div className="mt-7 space-y-3 relative">
        {/* From */}
        <div>
          <label className="text-text-50 text-xs font-medium mb-1.5 block">From</label>
            ...
            >
              <span className="size-3.5 rounded-full bg-primary-500/10 flex items-center justify-center">
                <span className="[&_svg]:fill-none [&_path]:fill-none">
                  <QuestionMarkCircleStrokeRounded size={9} strokeWidth={1.8} className="text-primary-500" />
                </span>
              </span>
        ...
        {/* To */}
        <div>
          <label className="text-text-50 text-xs font-medium mb-1.5 block">To</label>
          <div className="flex items-stretch rounded-xl border border-base-100 overflow-hidden focus-within:border-primary-500 transition">
            <CurrencySelector
              value={state.to}
        ...
        {open && (
          <>
            ...
            <motion.div
              ...
              transition={{ duration: 0.15 }}
              className="absolute top-full left-0 right-0 mt-1.5 rounded-xl border border-base-100 bg-background-50 shadow-xl z-20 p-1.5"
            >
              {COUNTRIES.map(c => (
                <button
                  key={c.code}
        ...
        {open && (
          <>
            ...
            <motion.div
              initial={{ opacity: 0, y: -4 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -4 }}
              transition={{ duration: 0.15 }}
              className="absolute top-full left-0 right-0 mt-1.5 rounded-xl border border-base-100 bg-background-50 shadow-xl z-20 p-1.5 max-h-64 overflow-y-auto"
            >
              {BANKS.map(b => (
                <button
                  key={b}
...
function DetailsScreen({ state, dispatch }: { state: FlowState; dispatch: React.Dispatch<FlowAction> }) {
  ...
  return (
    <div className="max-w-md w-full mx-auto py-8">
      ...
      <h2 className="text-title-50 text-xl font-semibold">Who are you going to send it to?</h2>
...
function ReviewScreen({
  ...
  return (
    <div className="max-w-md w-full mx-auto py-8">
      ...
      <h2 className="text-title-50 text-xl font-semibold">Review details</h2>
      ...
      >
        ...
        <div className="h-9 bg-[#1e1e1e] flex items-center px-4 gap-2 shrink-0">
          ...
          <div className="flex-1 flex justify-center">
            <span className="text-white/40 text-[11px]">CrossBorder — Monato · Quote step {state.step}/4</span>
          </div>
        </div>
        ...
        <div className="flex-1 flex overflow-hidden bg-background-50">
          ...
          <main className="flex-1 flex flex-col overflow-hidden">
            {/* Top stepper bar */}
            <div className="bg-background-50 border-b border-base-100 px-7 py-3 flex items-center justify-between shrink-0">
              <div className="w-24" />
              <Stepper current={state.step} />
              <div className="w-24 flex justify-end">
                <button
                  onClick={onExit}
                  className="size-7 rounded-full border border-base-200 hover:bg-background-soft-50 flex items-center justify-center text-text-200 hover:text-title-50 transition text-sm"
                  aria-label="Close"
                >
                  ✕
                </button>
            ...
            {/* Screens */}
            <div className="flex-1 overflow-y-auto bg-background-soft-50/40">
...
export function CrossBorderPrototype() {
  ...
  return (
    <div>
      <div className="rounded-xl border border-base-100 bg-background-50 p-6">
        <div className="flex items-center gap-4 mb-5">
          <div className="size-12 rounded-xl bg-gradient-to-br from-primary-500 to-primary-600 flex items-center justify-center shadow-lg shadow-primary-500/30">
            <span className="[&_svg]:fill-none [&_path]:fill-none">
              <DollarCircleStrokeRounded size={22} strokeWidth={1.4} className="text-white" />
            </span>
          </div>
          <div>
            <h2 className="text-title-50 text-base font-semibold">CrossBorder v1.0</h2>
            <p className="text-text-100 text-xs mt-0.5">Flujo completo de transferencia internacional</p>
          </div>
          <Badge color="primary" size="sm" className="ml-auto">Beta</Badge>
        </div>

        <p className="text-text-100 text-sm leading-relaxed">
          Prototipo de transferencia cross-border con 4 pasos: cotización con conversión bidireccional, datos del destinatario,
          revisión con selección de cuenta de fondeo, y confirmación. Incluye soporte para MXN, USD, EUR y monedas cripto (USDC, USDT).
        </p>

        <div className="mt-5 flex gap-2">
          <Button onClick={() => setOpen(true)}>
            <span className="[&_svg]:fill-none [&_path]:fill-none">
              <Home2StrokeRounded size={13} strokeWidth={1.4} />
            </span>
            Abrir prototipo
          </Button>
        </div>
