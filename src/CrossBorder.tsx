...
const CURRENCIES: Currency[] = [
  ...
  { code: "EUR", label: "Euro", flag: "eu" },
  {
    ...
    isCrypto: true,
    cryptoColor: "from-blue-400 to-blue-600",
  },
  {
    code: "USDC/P",
    label: "USDC on Polygon",
    isCrypto: true,
    cryptoColor: "from-purple-400 to-purple-600",
  },
  {
    code: "USDT/P",
    label: "USDT on Polygon",
    isCrypto: true,
    cryptoColor: "from-emerald-400 to-emerald-600",
  },
];
...
function CurrencySelector({
  ...
  return (
    <div className="relative">
      <button
        onClick={() => setOpen((v) => !v)}
        className="flex items-center gap-2.5 px-3 h-12 rounded-l-xl bg-background-soft-50 border-r border-base-100 min-w-[150px] transition hover:bg-background-soft-100"
      >
        {selected ? (
          <>
          ...
          <span className="text-text-200 text-sm">Select currency</span>
        )}
        <motion.span
          animate={{ rotate: open ? 180 : 0 }}
          className="[&_svg]:fill-none [&_path]:fill-none ml-auto"
        >
          <ChevronDownStrokeRounded
            size={13}
            strokeWidth={1.6}
        ...
        {open && (
          <>
            ...
            <motion.div
              ...
              className="absolute top-full left-0 mt-2 w-56 rounded-xl border border-base-100 bg-background-50 shadow-xl z-[999] p-1.5"
            >
              {options.map((c) => (
                <button
                  key={c.code}
                  ...
                  className={`w-full flex items-center gap-2.5 px-2.5 py-2 rounded-lg text-left transition hover:bg-background-soft-50 ${
                    value === c.code ? "bg-primary-500/5" : ""
                  }`}
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-text-200 text-[11px] truncate">
                      {c.label}
                    </p>
                  </div>
                </button>
      ...
      {STEPS.map((step, i) => {
        ...
        return (
          <div key={step.id} className="flex items-center gap-1">
              ...
              >
                {isCompleted ? (
                  <span className="[&_svg]:fill-none [&_path]:fill-none">
                    <CheckStrokeRounded size={12} strokeWidth={2.2} />
                  </span>
                ) : (
                  step.id
                )}
            ...
            {i < STEPS.length - 1 && (
              <span
                className={`mx-2 [&_svg]:fill-none [&_path]:fill-none transition-colors ${
...
function QuoteScreen({
  ...
  const handleFromChange = (v: string) => {
    ...
    if (clean && both) {
      ...
      dispatch({
        type: "set",
        field: "toAmount",
        value: (num * RATE_MXN_USD).toFixed(2),
      });
    } else if (!clean) {
  ...
  const swap = () => {
    ...
    const oldFromAmt = state.fromAmount;
    const oldToAmt = state.toAmount;
    dispatch({ type: "set", field: "from", value: oldTo });
    dispatch({ type: "set", field: "to", value: oldFrom });
    dispatch({ type: "set", field: "fromAmount", value: oldToAmt });
    dispatch({ type: "set", field: "toAmount", value: oldFromAmt });
  };

  return (
    <div className="max-w-lg w-full mx-auto py-12">
      {/* Stepper integrado en la pantalla */}
      ...
      <p className="text-text-100 text-sm mt-1.5">
        Type in either field to get a quote.
      </p>

      <div className="mt-7 space-y-3 relative">
        {/* From */}
        <div>
          <label className="text-text-50 text-xs font-medium mb-1.5 block">
            From
          </label>
          <div className="flex items-stretch rounded-xl border border-base-100 overflow-hidden focus-within:border-primary-500 transition">
            <CurrencySelector
              value={state.from}
              onChange={(v) =>
                dispatch({ type: "set", field: "from", value: v })
              }
              exclude={state.to}
            />
            <input
              ...
              placeholder="00.0"
              className="flex-1 h-12 px-4 bg-background-soft-50 text-right text-title-50 text-xl font-semibold tracking-tight focus:outline-none placeholder:text-text-200"
            />
          </div>
          {both && state.fromAmount && (
            <motion.div
              ...
              className="mt-1.5 flex items-center gap-1 text-[11px] text-text-100"
            >
              <span className="size-3.5 rounded-full bg-primary-500/10 flex items-center justify-center">
                <span className="[&_svg]:fill-none [&_path]:fill-none">
                  <QuestionMarkCircleStrokeRounded
                    size={9}
                    strokeWidth={1.8}
              ...
              Includes processing fee: ${PROCESSING_FEE_MXN.toFixed(4)}{" "}
              {state.from}
            </motion.div>
          )}
        </div>
        ...
        {/* Swap button */}
        <div className="flex justify-end -my-1.5 relative z-10">
          <motion.button
            whileHover={{ scale: 1.05 }}
            whileTap={{ scale: 0.95, rotate: 180 }}
            onClick={swap}
            className="flex items-center gap-1.5 h-7 px-3 rounded-full border border-base-100 bg-background-50 text-xs text-title-50 shadow-sm transition hover:bg-background-soft-50"
          >
            <span className="[&_svg]:fill-none [&_path]:fill-none text-primary-500">
              <RefreshDollar1StrokeRounded size={12} strokeWidth={1.6} />
            </span>
            Swap
          </motion.button>
        </div>
          ...
          >
            <div className="flex gap-2.5 rounded-xl border border-yellow-200 bg-yellow-50 px-3.5 py-3">
              ...
              <p className="text-yellow-800 text-xs leading-relaxed">
                The exchange rate is dynamic. Once you review details and agree,
                this quote will expire in 2 hrs.{" "}
                <a href="#" className="underline font-medium">
                  Read more
                </a>{" "}
                about after-hours rates.
              </p>
            </div>
          </motion.div>
        )}
        ...
        <p className="text-text-100 text-xs">
          ...
          {hasAmount ? (
            <span className="text-title-50 font-medium">
              $1 {state.from} = ${RATE_MXN_USD} {state.to}
            </span>
          ) : (
...
function FormInput({
  ...
  return (
    <div>
      <label
        className={`text-xs font-medium mb-1.5 flex items-center gap-1 ${error ? "text-red-500" : "text-text-50"}`}
      >
        {label}
        {optional && (
          <span className="text-text-200 font-normal">(Optional)</span>
        )}
      </label>
      <input
        value={value}
        ...
        className={`w-full h-10 rounded-xl border px-3.5 text-sm text-title-50 bg-background-50 placeholder:text-text-200 focus:outline-none transition ${
          error
            ...
            : "border-base-100 focus:border-primary-500"
        }`}
      />
  ...
  return (
    <div className="relative">
      ...
      >
        ...
        <motion.span
          animate={{ rotate: open ? 180 : 0 }}
          className="[&_svg]:fill-none [&_path]:fill-none"
        >
          <ChevronDownStrokeRounded
            size={13}
            strokeWidth={1.6}
          ...
          <>
            ...
            >
              {COUNTRIES.map((c) => (
                ...
                >
                  <span
                    className={`fi fi-${c.flag} rounded-full`}
                    style={{
                      width: 18,
                      height: 18,
                      backgroundSize: "cover",
...
function BankSelector({
  ...
  return (
    <div className="relative">
      ...
      >
        <span
          className={`flex-1 text-sm ${value ? "text-title-50" : "text-text-200"}`}
        >
...
function DetailsScreen({
  ...
  const showAccountError =
    accountTouched && state.accountNumber.length > 0 && !accountValid;

  const canContinue =
    state.firstName && state.firstLastName && accountValid && state.bank;
      ...
      <div className="mt-6 space-y-5">
        ...
        <div>
          ...
          <CountrySelector
            value={state.country}
            onChange={(v) =>
              dispatch({ type: "set", field: "country", value: v })
            }
          />
        </div>
        ...
        {/* Personal data */}
        <div>
          <h3 className="text-title-50 text-sm font-semibold mb-3">
            Personal data
          </h3>
          <div className="space-y-3">
            <FormInput
              ...
              onChange={(v) =>
                dispatch({ type: "set", field: "firstName", value: v })
              }
            ...
            <FormInput
              ...
              onChange={(v) =>
                dispatch({ type: "set", field: "middleName", value: v })
              }
            ...
            <FormInput
              ...
              onChange={(v) =>
                dispatch({ type: "set", field: "firstLastName", value: v })
              }
            ...
            <FormInput
              ...
              onChange={(v) =>
                dispatch({ type: "set", field: "secondLastName", value: v })
              }
            ...
            <FormInput
              ...
              value={state.alias}
              onChange={(v) =>
                dispatch({ type: "set", field: "alias", value: v })
              }
              placeholder="Ex. Rob Main Account"
              optional
            />
          </div>
        </div>
        ...
        <div>
          ...
          <div className="space-y-3">
            <FormInput
              ...
              onChange={(v) => {
                dispatch({ type: "set", field: "accountNumber", value: v });
                setAccountTouched(true);
              }}
              placeholder={
                showAccountError
                  ? "Please enter a valid number"
                  : "Ex. 734180999000000006"
              }
              error={
                showAccountError ? "Please enter a valid number" : undefined
              }
            />
            <div>
              <label className="text-text-50 text-xs font-medium mb-1.5 block">
              ...
              <BankSelector
                value={state.bank}
                onChange={(v) =>
                  dispatch({ type: "set", field: "bank", value: v })
                }
              />
            </div>
          </div>
  ...
  return (
    <div className="max-w-lg w-full mx-auto py-12">
      ...
      <div className="mt-5 pt-5 border-t border-base-100">
        <p className="text-title-50 text-sm font-semibold mb-4">
          Transfer details
        </p>
        ...
        <div className="text-center space-y-3">
          <div>
            ...
            <div className="flex items-center justify-center gap-2 mt-1">
              ...
              <p className="text-title-50 text-lg font-semibold">
                ${state.fromAmount} {state.from}*
              </p>
            </div>
          </div>
          ...
          <div>
            <p className="text-text-100 text-xs">
              {state.firstName || "Recipient"} gets
            </p>
            <div className="flex items-center justify-center gap-2 mt-1">
        ...
        <div className="mt-5 space-y-1.5 text-xs">
          <div className="flex justify-between">
            <span className="text-text-100">Includes processing fee*</span>
            <span className="text-title-50">
              -${PROCESSING_FEE_MXN.toFixed(4)} {state.from}
            </span>
          </div>
          <div className="flex justify-between">
            <span className="text-text-100">Currency Rate</span>
            <span className="text-title-50">
              $1 {state.from} = ${RATE_MXN_USD} {state.to}
            </span>
          </div>
      ...
      <div className="mt-6 pt-6 border-t border-base-100">
        <p className="text-title-50 text-sm font-semibold mb-3">
          Funding method
        </p>
        ...
        <div className="space-y-2.5">
          ...
          <motion.div
            ...
            animate={{
              borderColor:
                state.fundingMethod === "monato" ? "#0894c8" : "#f4f4f5",
            }}
          ...
          >
            ...
            >
              <div
                className={`size-4 rounded-full border-2 mt-0.5 shrink-0 transition ${
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-text-100 text-xs mt-1.5 leading-relaxed">
                  Keep your balance ready by adding funds according to your
                  daily volume needs.
                </p>
                <p className="text-primary-500 text-[11px] mt-1.5 font-medium">
                  ...
                  <div className="p-3.5 space-y-2 bg-background-soft-50/40">
                    {MONATO_ACCOUNTS.map((acc) => {
                      ...
                      return (
                        ...
                        >
                          <div
                            className={`size-7 rounded-full ${acc.color} flex items-center justify-center text-white text-[9px] font-bold shrink-0`}
                          >
                            MXN
                          </div>
                          <div className="flex-1 min-w-0">
                            <p className="text-title-50 text-xs font-medium">
                              {acc.name}
                            </p>
                            <p className="text-text-200 text-[10px]">
                              CLABE: **** {acc.clabe}
                            </p>
                          </div>
                          <div className="text-right shrink-0">
                            <p
                              className={`text-xs font-semibold ${acc.insufficient ? "text-text-200 line-through" : "text-title-50"}`}
                            >
                              $
          ...
          <motion.button
            ...
            animate={{
              borderColor:
                state.fundingMethod === "spei" ? "#0894c8" : "#f4f4f5",
            }}
            onClick={() =>
              dispatch({ type: "set", field: "fundingMethod", value: "spei" })
            }
          ...
          >
            ...
            <div className="flex-1">
              <div className="flex items-center gap-2">
                ...
                <p className="text-title-50 text-sm font-medium">
                  Fund with external SPEI transfer
                </p>
              </div>
              <p className="text-text-100 text-xs mt-1.5 leading-relaxed">
              ...
              <p className="text-primary-500 text-[11px] mt-1.5 font-medium">
                Usually 1–2 min · Standard SPEI fees apply
              </p>
            </div>
          </motion.button>
        </div>
      ...
      <div className="mt-6 pt-6 border-t border-base-100">
        ...
        <div className="space-y-2 text-xs">
          <div className="flex justify-between">
            <span className="text-text-100">Name of the account holder</span>
            <span className="text-title-50">{fullName || "—"}</span>
          </div>
          <div className="flex justify-between">
            <span className="text-text-100">Country</span>
            <span className="text-title-50">{country.label}</span>
          </div>
          <div className="flex justify-between">
            <span className="text-text-100">Account Number</span>
            <span className="text-title-50">{state.accountNumber}</span>
          </div>
          <div className="flex justify-between">
            <span className="text-text-100">Bank Name</span>
            <span className="text-title-50">{state.bank}</span>
          </div>
        </div>
      </div>
      ...
      <div className="mt-5 flex gap-2.5 rounded-xl border border-yellow-200 bg-yellow-50 px-3.5 py-3">
        ...
        <p className="text-yellow-800 text-xs leading-relaxed">
          The exchange rate will be locked for 2 hours after you agree with the
          quote and details.{" "}
          <a href="#" className="underline font-medium">
            Read more
          </a>{" "}
      ...
      <div className="mt-6 flex justify-center">
        <Button
          ...
          disabled={
            !state.fundingMethod ||
            (state.fundingMethod === "monato" && !state.selectedAccount)
          }
    ...
    <div className="max-w-lg w-full mx-auto py-12">
      ...
      <div className="text-center">
        ...
        <p className="text-text-100 text-sm mt-4 leading-relaxed max-w-sm mx-auto">
          Your balance has been successfully used to fund this transaction. We
          are now processing your cross-border payment, and it will be completed
          shortly.
        </p>
      </div>
      ...
      {account && (
        <div className="mt-6 pt-6 border-t border-base-100">
          ...
          <div className="space-y-2 text-xs">
            <div className="flex justify-between">
              <span className="text-text-100">Name of the account</span>
              <span className="text-title-50">{account.name}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-text-100">CLABE account</span>
              <span className="text-title-50">**** {account.clabe}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-text-100">Bank</span>
              <span className="text-title-50">Fincopay</span>
            </div>
          </div>
        </div>
      )}
  ...
  return (
    <Modal open={open} onClose={onCancel}>
      <div className="w-full max-w-sm rounded-2xl bg-background-50 shadow-xl p-6 text-center">
        ...
        <p className="text-text-100 text-sm mt-2">
  ...
  const items = [
    {
      id: "quote" as const,
      label: "Quote",
      icon: DollarCircleStrokeRounded,
      disabled: false,
    },
    {
      id: "transactions" as const,
      label: "All transactions",
      icon: Layers1StrokeRounded,
      disabled: true,
    },
    {
      id: "accounts" as const,
      label: "Accounts",
      icon: UserMultiple4StrokeRounded,
      disabled: true,
    },
  ];
    ...
    <aside className="h-full w-60 shrink-0 flex flex-col border-r border-base-100 bg-background-50">
            ...
            >
              ...
              <span className="[&_svg]:fill-none [&_path]:fill-none relative z-10">
                <Icon
                  size={15}
                  strokeWidth={1.4}
                  className={isActive ? "text-primary-500" : "text-text-200"}
                />
              </span>
              ...
              {disabled && (
                <Badge
                  color="gray"
                  size="sm"
                  className="ml-auto relative z-10 text-[9px]"
                >
                  Pronto
                </Badge>
              )}
      ...
      <div className="border-t border-base-100 px-4 py-3 flex items-center gap-2.5">
        ...
        <div className="min-w-0">
          <p className="text-title-50 text-xs font-medium truncate">
            Kathryn Murphy
          </p>
          <p className="text-text-200 text-[11px] truncate">
...
function CBDesktop({ onExit }: { onExit: () => void }) {
  ...
  const ScreenComponent = (() => {
    switch (state.step) {
      case 1:
        return <QuoteScreen state={state} dispatch={dispatch} />;
      case 2:
        return <DetailsScreen state={state} dispatch={dispatch} />;
      case 3:
        return (
          <ReviewScreen
            state={state}
      ...
      case 4:
        return <FundScreen state={state} dispatch={dispatch} onExit={onExit} />;
    }
  })();
    ...
    <div className="w-full h-full overflow-hidden bg-gradient-to-br from-slate-800 to-slate-900 flex items-center justify-center p-4">
      ...
      >
        ...
        <div className="h-9 bg-[#1e1e1e] flex items-center px-4 gap-2 shrink-0">
          ...
          >
            <span className="text-red-900 text-[8px] font-bold opacity-0 group-hover:opacity-100">
              ✕
            </span>
          </button>
          ...
          <div className="flex-1 flex justify-center">
            <span className="text-white/40 text-[11px]">
              CrossBorder — Monato · {STEPS[state.step - 1].label}
            </span>
          </div>
        </div>
        ...
        <div className="flex-1 flex overflow-hidden bg-background-50">
          ...
          <main className="flex-1 flex flex-col overflow-hidden">
            {/* Screens — stepper va dentro de cada screen */}
      ...
      <div className="rounded-xl border border-base-100 bg-background-50 overflow-hidden">
        <div className="border-b border-base-100 bg-background-soft-50 px-4 py-2.5 flex items-center justify-between">
          <span className="text-text-200 text-[11px] font-medium uppercase tracking-widest">
            CrossBorder v1.0 — Prototipo navegable
          </span>
          ...
          <div className="flex-1 space-y-3">
            <div>
              <h3 className="text-title-50 text-base font-semibold">
                CrossBorder — Transferencia Internacional
              </h3>
              <p className="text-text-100 text-sm mt-1">
                Flujo completo de 4 pasos: cotización con conversión
                bidireccional MXN/USD/EUR/Cripto, datos del destinatario,
                revisión con selección de cuenta y confirmación.
              </p>
            </div>
